...
const Sourcing = (props) => {
  const SourcingSection = styled.div`
  ...
  return (
    ...
    >
      {!props.loading ? (
        <div>
          <SourcingSection>
            <TitleDiv>{props.sourcingGetData.data.attributes.title}</TitleDiv>
            <Intro>{props.sourcingGetData.data.attributes.intro}</Intro>
          </SourcingSection>
          <CardDiv>
            {props.sourcingGetData.included[0].attributes.contents.map(
              (card, i) => {
                return (
                  <Card>
                    <h3 style={{ display: "none" }}>{i + 300}</h3>
                    <TopDiv>
                      <TopLeftDiv>
