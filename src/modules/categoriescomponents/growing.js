...
const Growing = (props) => {
  const GrowingSection = styled.div`
  ...
  return (
    ...
    >
      {!props.loading ? (
        <div>
          <GrowingSection>
            <TitleDiv>{props.growingGetData.data.attributes.title}</TitleDiv>
            <Intro>{props.growingGetData.data.attributes.intro}</Intro>
          </GrowingSection>
          <CardDiv>
            {props.growingGetData.included[0].attributes.contents.map(
              (card) => {
                return (
                  <Card>
                    <h3 style={{ display: "none" }}>{9999}</h3>
                    <TopDiv>
                      <TopLeftDiv>
