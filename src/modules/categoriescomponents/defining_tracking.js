...
const Defining = (props) => {
  const DefiningSection = styled.div`
  ...
  return (
    ...
    >
      {!props.loading ? (
        <div>
          <DefiningSection>
            <TitleDiv>{props.definingGetData.data.attributes.title}</TitleDiv>
            <Intro>{props.definingGetData.data.attributes.intro}</Intro>
          </DefiningSection>
          <CardDiv>
            {props.definingGetData.included[0].attributes.contents.map(
              (card, i) => {
                return (
                  <Card>
                    <h3 style={{ display: "none" }}>{i + 100}</h3>
                    <TopDiv>
                      <TopLeftDiv>
